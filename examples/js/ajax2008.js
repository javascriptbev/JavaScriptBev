...
// if( pos == 0 )
// {
    // curURL = "mk:@MSITStore:" + curURL.substring(14,curURL.length-1);
...
//zhaoah 20091127 firefox 滚动条bug
//            if (oBanner != null)
//            {
...
/* Begin Copy Code zhaoah,用于当前页有多个复制粘贴代码*/
// Copy to ClipBroad,目前仅支持 ie和Firefox！
			function copyToClipBoard(index){
				var pre = document.getElementsByTagName("pre");
				var clipBoardContent = "";
				if(window.ActiveXObject) {	//如果为ie浏览器
					clipBoardContent += pre[index].innerText;
				}
				//firefox浏览器
				else {
					if(pre[index].hasChildNodes) {
						//var element = pre[index].firstChild;
						//var clipBoardContent=element.innerHTML;
						
						for(var i = 0; i < pre[index].childNodes.length; i++) {
							if(pre[index].childNodes[i].innerHTML) {
								clipBoardContent += pre[index].childNodes[i].innerHTML;
							}
							else {
								clipBoardContent += pre[index].childNodes[i].nodeValue;
							}
						}
					}
					else {
						var element = pre;
						var clipBoardContent=element[index].innerHTML;
					}
					//var element = document.getElementsByTagName("pre");
					//var clipBoardContent=element[index].innerHTML;
					var relt = /&lt;/g;
					clipBoardContent = clipBoardContent.replace(relt,"<");
					var regt = /&gt;/g;
					clipBoardContent = clipBoardContent.replace(regt,">");
					var rbr = /<br>/g;
					clipBoardContent = clipBoardContent.replace(rbr,"\n");
					var rbigbr = /<BR>/g;
					clipBoardContent = clipBoardContent.replace(rbigbr,"\n");

					var rbnsp = /&nbsp;/g;
					clipBoardContent = clipBoardContent.replace(rbnsp," ");
				}
       //clipBoardContent+="ＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸ";
